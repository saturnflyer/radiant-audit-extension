    ...
    def self.included(base)
      base.class_eval do
        ...
        audit_event :create do |event|
          "#{event.user_link} created <a href='#{event.auditable.name}'>#{event.auditable.path}</a>"
        end
        ...
        audit_event :update do |event|
          ...
          log_message = "#{event.user_link} updated <a href='#{event.auditable.name}'>#{event.auditable.path}</a>"
        ...
        audit_event :destroy do |event|
          "#{event.user_link} deleted <a href='#{event.auditable.name}'>#{event.auditable.path}</a>"
        end
      end
    end
